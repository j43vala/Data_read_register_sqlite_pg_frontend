...
from models import Device, Parameter  # Import the Device and Parameter models
from db import db
...
class DeviceResourceList(Resource):
    def get(self):
        """Retrieve all devices with their connected Parameters."""
        status = 0
...
class DeviceResource(Resource):
    def get(self, id):
        """Retrieve a specific device with its connected parameters."""
        status = 0
    ...
    def delete(self, id):
        """Delete a specific device and deactivate associated parameters."""
        status = 0
        return_fields = ["name"]
        device = db.session.query(Device).filter_by(id=id).first()
        ...
        status = 1
        return make_response(jsonify({"status": status, "message": "Device and associated parameters have been deactivated."}), 200)
